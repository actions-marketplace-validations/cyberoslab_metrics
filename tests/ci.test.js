...
describe("Check file changes (checkout your files if needed)", () => {
  ...
  if (!["cyberoslab"].includes(process.env.PR_AUTHOR)) {
...
describe("Check template changes", () => {
  test("Use community templates instead (see https://github.com/cyberoslab/metrics/tree/master/source/templates/community)", async () => void expect((await diff()).filter(edited => /^sources[/]templates[/]/.test(edited) && /^source[/]templates[/](?:classic|terminal|markdown|repository|community)[/][\s\S]*$/.test(edited)).length).toBe(0))
})
