...
> 💡 For security reasons, it is advised to always use the least amount of scopes. It is possible to prevent security issues by [forking this repository](https://github.com/cyberoslab/metrics/fork) and using it in your workflow instead (more information available in step 3)
...
![Plugin error example](https://github.com/cyberoslab/metrics/blob/examples/metrics.plugin.error.svg)
...
- `@master`/`@main`
  ...
  - ➖ Jobs may fail occasionally (watch [issues](https://github.com/cyberoslab/metrics/issues) and [discussions](https://github.com/cyberoslab/metrics/discussions) for bug tracking)
- `@{fork}`
  - ✔️ Secure as you're in control
  - ✔️ Advised when using additional scopes in personal access token
  - ➖ Manual updates (watch new [releases](https://github.com/cyberoslab/metrics/releases) for updates)
- `@v{x}.{x}`
